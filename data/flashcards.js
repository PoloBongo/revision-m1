...
window.FLASHCARDS = {
  ...
  planif: [
    ...
    { front: "MPM — principe", back: "Méthode des Potentiels Métra (B. Roy, 1958). Tâches = sommets, contraintes = arcs. Calcule dates au plus tôt/tard et marges." },
    { front: "PERT vs MPM", back: "PERT : tâches = arcs, événements = sommets. MPM : tâches = sommets, durées = arcs. Les deux calculent le chemin critique." },
    { front: "Date au plus tôt", back: "T0 = 0. Pour j : T_tôt(j) = Max(T_tôt(i) + durée i→j) sur tous les prédécesseurs i. On avance de gauche à droite." },
    { front: "Date au plus tard", back: "On initialise la fin : T_tard(fin) = T_tôt(fin). Pour i : T_tard(i) = Min(T_tard(j) − durée i→j) sur tous les successeurs j." },
    { front: "Marge totale", back: "Retard max sans retarder la fin du projet. MT = T_tard(j) − T_tôt(i) − Durée(i→j). Sur le chemin critique = 0." },
    { front: "Marge libre", back: "Retard max sans retarder la date au plus tôt de la tâche suivante. ML = Min(T_tôt(j) − T_tôt(i) − Durée i→j)." },
    { front: "Chemin critique — propriétés", back: "Toujours ≥ 1 chemin critique. Marge nulle sur toutes ses tâches. Détermine la durée minimale du projet. Tout retard retarde la fin." },
    { front: "Méthode 3 points", back: "Estimation de durée : (Pessimiste + 4×Probable + Optimiste) / 6. Tient compte de l'incertitude. Utilisée dans MS Project." },
    { front: "Liaison FD (Fin-Début)", back: "La plus courante : une tâche B commence quand A est terminée. Autres liaisons : DD (Début-Début), FF (Fin-Fin), DF (Début-Fin)." },
    { front: "WBS — règle des 100%", back: "La WBS doit inclure 100% du travail du projet : tous livrables internes, externes et intermédiaires, sans oublier la gestion de projet." },
    { front: "Jalon (milestone)", back: "Tâche de durée nulle (0 j) marquant un événement clé : fin de phase, livraison, validation client." },
  ],
  cycles: [
    ...
    { front: "Cascade (Waterfall) — limites", back: "Phases séquentielles ; une erreur de compréhension n'est détectée qu'aux tests fonctionnels → coût de correction élevé. Solution : le cycle en V." },
    { front: "Cycle en spirale — 4 phases", back: "1. Détermination objectifs/alternatives/contraintes. 2. Analyse des risques. 3. Développement et vérification. 4. Revue et planification du cycle suivant." },
    { front: "Cycle en V — 8 étapes", back: "↓ Besoins → Specs fonctionnelles → Specs techniques → Codage ↑ Tests unitaires → Tests intégration → Validation fonctionnelle → Recette/MEP." },
    { front: "AMOA", back: "Assistant Maîtrise d'Ouvrage : aide la MOA à définir et piloter le projet. Rôle de conseil. Ne décide pas à la place de la MOA." },
    { front: "AMOE", back: "Assistant Maîtrise d'Œuvre : renforce les équipes MOE (conception technique, développement, tests, déploiement)." },
    { front: "Effet tunnel", back: "La MOA est exclue des phases techniques → produit livré non conforme aux besoins évolutifs. 80% des fonctionnalités développées seraient inutiles (Standish Group)." },
    { front: "Manifeste Agile — 4 valeurs", back: "1. Individus > processus. 2. Logiciel fonctionnel > documentation. 3. Collaboration client > négociation contractuelle. 4. Adaptation au changement > plan." },
    { front: "12 principes agiles (résumé)", back: "Satisfaction client, accepter le changement, livraisons fréquentes, collaboration quotidienne, motivation, communication face à face, logiciel = mesure de progrès, rythme durable, excellence technique, simplicité, équipes auto-organisées, rétrospective régulière." },
    { front: "Lean Management", back: "Toyota (1990) : 'gestion fine' → éliminer les gaspillages. 14 principes. Résultat : ~30% de réduction des coûts. Base du Scrum." },
    { front: "Kanban — principe", back: "Toyota : étiquettes (kanbans) représentant les besoins client. Tableau (À faire / En cours / Terminé). Flux tiré. WIP limité → moins de surcharge." },
    { front: "XP — eXtreme Programming", back: "13 pratiques : TDD, pair programming (Driver + Partner), Planning Poker, refactoring, intégration continue, releases fréquentes, standards de code, tests de recette…" },
    { front: "SCRUM — origine", back: "Takeuchi & Nonaka (1986). Ken Schwaber formalise (1995). Schwaber + Mike Beedle publient en 2001. Mêlée rugby = cohésion d'équipe vers un but commun." },
    { front: "Équipe SCRUM (3 rôles)", back: "Product Owner (vision produit + backlog), Scrum Master (facilitateur), Équipe de développement (auto-organisée, 5-6 personnes, hétérogène)." },
    { front: "Product Owner", back: "Porte la vision produit. Rédige et priorise les User Stories (Product Backlog). Définit la valeur métier. Ne peut pas être un développeur." },
    { front: "User Story — format", back: "'En tant que [rôle], je peux [action] afin de [bénéfice]'. Estimée en effort (Planning Poker) et priorisée par valeur métier (Priority Poker)." },
    { front: "Product Backlog", back: "Liste priorisée de toutes les User Stories souhaitées. Géré par le Product Owner, mis à jour en continu. Ordonné par valeur métier décroissante." },
    { front: "Sprint", back: "Itération fixe de 2 à 4 semaines produisant un incrément potentiellement livrable. Le périmètre est figé dès le début du Sprint." },
    { front: "Sprint Backlog", back: "Sélection de User Stories du Product Backlog pour le Sprint courant, découpées en tâches : À faire / En cours / Terminée." },
    { front: "Daily Scrum (Scrum Meeting)", back: "15 min, debout, quotidien. Chaque membre répond : Qu'ai-je fait hier ? Que vais-je faire aujourd'hui ? Quelles difficultés ai-je rencontrées ?" },
    { front: "Revue de Sprint", back: "Démo du livrable en fin de Sprint (max 4h pour un Sprint d'1 mois). Product Owner valide ou non. Retours immédiats des utilisateurs — supprime l'effet tunnel." },
    { front: "Rétrospective de Sprint", back: "Amélioration continue (max 3h). Post-it : vert = bien, orange = à améliorer, rouge = à corriger. Animée par le Scrum Master." },
    { front: "Burn Down Chart", back: "Reste à faire (Y) vs durée du Sprint (X). Mis à jour après chaque Daily Scrum. Montre si l'équipe est en avance ou en retard." },
    { front: "Burn Up Chart", back: "Valeur métier développée (Y) vs durée (X). Quand 80% de la valeur est atteinte → décision de continuer ou non le projet." },
    { front: "Sprint 0", back: "Période de préparation : constitution équipe, écriture/priorisation Product Backlog, configuration environnement dev, formation Scrum." },
    { front: "Planning Poker", back: "Estimation collégiale en effort. Cartes Fibonacci. Tous révèlent simultanément → discussion si divergence → recommencer jusqu'au consensus." },
    { front: "5 événements SCRUM", back: "Sprint · Planification de Sprint (8h max) · Daily Scrum (15 min) · Revue de Sprint (4h max) · Rétrospective (3h max)." },
    { front: "5 artefacts SCRUM", back: "Product Backlog · Sprint Backlog · Burn Down Chart · Burn Up Chart · Courbe de température (moral de l'équipe)." },
    { front: "Vélocité SCRUM", back: "Nombre de points Story traités par Sprint. Sert à planifier les Releases (succession de Sprints vers un objectif livrable)." },
  ],
  management: [
    ...
    { front: "QQOQCP", back: "Qui ? Quand ? Où ? Quoi ? Comment ? Combien ? Pourquoi ? — méthode de questionnement structuré pour analyser toute situation de projet." },
    { front: "Comité de Pilotage (COPIL)", back: "Équipe transversale nommée au lancement : sponsor, directeurs, experts, chef de projet. Rend compte au Comité Directeur. Dissous à la fin du projet." },
    { front: "Matrice des risques", back: "Axes Probabilité (P) × Gravité (G). Importance = P×G. Qualificatifs : 1 (Très faible), 2 (Faible), 4 (Grand), 8 (Très grand). Risque faible : PG ≤ 4." },
    { front: "Diagramme Ishikawa (6M)", back: "Arête de poisson : identifie les causes d'un problème selon 6 familles : Matière, Méthode, Milieu, Matériel, Main d'œuvre, Management." },
    { front: "4 étapes gestion des risques", back: "1. Identification. 2. Quantification (P×G ou valeur monétaire). 3. Prévision / plans d'urgence. 4. Surveillance et gestion continue." },
    { front: "Recette — définition", back: "Opération par laquelle le client reconnaît que le produit est conforme à la commande et exploitable. Étapes : prépa → cahier de recette → jeux d'essai → exécution → VABF → VSR." },
    { front: "VABF / VSR", back: "VABF : Vérification d'Aptitude au Bon Fonctionnement (déploiement pilote). VSR : Vérification de Service Régulier (exploitation globale). Le projet est terminé." },
    { front: "3 types de ressources", back: "Travail (personnes + équipements), Matériel (consommables : ampoules, papier…), Coût (charges financières : billet d'avion consultant)." },
    { front: "Matrice Eisenhower", back: "Classement des tâches : Urgent+Important → faire immédiatement. Important non-urgent → planifier. Urgent non-important → déléguer. Ni l'un ni l'autre → éliminer." },
    { front: "Suivi de projet", back: "Vérifier que l'avancement est conforme aux prévisions et corriger les retards. Incombe au chef de projet. Réunions périodiques + compte-rendu des écarts." },
    { front: "Valeur acquise (CBTE)", back: "Méthode de suivi : CBTP = budget planifié, CRTE = coût réel, CBTE = valeur budgétaire du réalisé. Courbe en S : CRTE vs CBTE = écart de productivité." },
    { front: "Définition d'un projet (AFNOR)", back: "'Un effort unique mettant en œuvre des moyens (humains, matériels, services) pour atteindre un objectif dans des délais fixés.' — Caractéristiques : unique, durée, complexité, variété d'intervenants." },
  ],
  ethique: [
